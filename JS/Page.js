...
function showFilter(AppPath) 
{
	...
	var Path = AppPath + "Pages Administrativa/Filtro.aspx?TableName=" + document.getElementById("__TABLENAME").value + "&DatabaseName=" + document.getElementById("__DATABASENAME").value + "&PageName=" + document.getElementById("__PAGENAME").value + "&cp=" + path64;
		...
		if (CommandName.toLowerCase() == "remove" || CommandName.toLowerCase() ==  "removelistitem")
		{
			Swal.fire({
				...
				if (result.isConfirmed) {
					ExecuteCommandRequest(CommandName, TargetControl, false, Parameters);
				}
			})
			return false;
		}
