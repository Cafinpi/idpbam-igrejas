...
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('CONS_ENVIAR_IDPBNACIONAL') AND sysstat & 0xf = 2)
DROP VIEW [CONS_ENVIAR_IDPBNACIONAL]
GO
...
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('CONS_PAGAR_TRIBUTOS') AND sysstat & 0xf = 2)
DROP VIEW [CONS_PAGAR_TRIBUTOS]
GO
...
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('CONS_TRIBUTOS') AND sysstat & 0xf = 2)
DROP VIEW [CONS_TRIBUTOS]
GO
...
/*------------------------------------------------------------*/
 ...
 CREATE TABLE [TB_PARAMETRO](
	[PAR_CAT_TRANSF]                       bigint               DEFAULT 0 NULL,
	[CC_SAIDA_CONTRIBUICAOMM]              bigint               DEFAULT 0 NOT NULL,
	[PAR_CENTROCUSTO]                      bigint               DEFAULT 0 NOT NULL,
	[PERC_REPASSE_AM]                      decimal (10,2)       DEFAULT 0 NULL,
	[PERC_REPASSE_NACIONAL]                decimal (10,2)       DEFAULT 0 NULL,
	[CAT_MISSOES_AM]                       bigint               DEFAULT 0 NULL,
	[CAT_MISSOES_NACIONAL]                 bigint               DEFAULT 0 NULL
		CONSTRAINT [PK_TB_PARAMETRO] PRIMARY KEY CLUSTERED
		(
			[CC_SAIDA_CONTRIBUICAOMM]
		) WITH FILLFACTOR = 90
)
GO
...
/*------------------------------------------------------------*/
/*                      Criação de VIEW                      */
/*            CONS_ENVIAR_IDPBNACIONAL            */
/*------------------------------------------------------------*/

 CREATE VIEW [CONS_ENVIAR_IDPBNACIONAL]AS(
select [CONS_FECHAMENTO_DETALHADO].[CAT_ID], [CONS_FECHAMENTO_DETALHADO].[CAT_NOME], [CONS_FECHAMENTO_DETALHADO].[GC_ID], [CONS_FECHAMENTO_DETALHADO].[GC_NOME], [CONS_FECHAMENTO_DETALHADO].[mesReferencia], [CONS_FECHAMENTO_DETALHADO].[anoReferencia], [CONS_FECHAMENTO_DETALHADO].[TotalReceita], [CONS_FECHAMENTO_DETALHADO].[TotalDespesa]
 from [CONS_FECHAMENTO_DETALHADO]

 where ([CONS_FECHAMENTO_DETALHADO].[GC_ID] = 4)  and mesReferencia = 1  and TotalDespesa is not null
)
GO
...
/*------------------------------------------------------------*/
 ...
 CREATE VIEW [CONS_TRIBUTOS]AS(
select [TB_LANCAMENTO].[GC_ID], [TB_LANCAMENTO].[CAT_ID], [TB_LANCAMENTO].[mesReferencia], [TB_LANCAMENTO].[anoReferencia], Sum([TB_LANCAMENTO].[LANC_VALOR]) as VALOR_PAGO
 from [TB_LANCAMENTO]
 where (((([TB_LANCAMENTO].[GC_ID] = 4) and [TB_LANCAMENTO].[CAT_ID] = 1) or [TB_LANCAMENTO].[GC_ID] = 5) and [TB_LANCAMENTO].[CAT_ID] = 1) 
group by [TB_LANCAMENTO].[GC_ID], [TB_LANCAMENTO].[CAT_ID], [TB_LANCAMENTO].[mesReferencia], [TB_LANCAMENTO].[anoReferencia] 
)
GO
...
/*------------------------------------------------------------*/
 ...
 CREATE VIEW [CONS_PAGAR_TRIBUTOS]AS(
Select VV_SOMA_RECEITAOPERACIONAL.mesReferencia,
  VV_SOMA_RECEITAOPERACIONAL.anoReferencia,
  VV_SOMA_RECEITAOPERACIONAL.TotalReceita,
  VV_SOMA_RECEITAOPERACIONAL.repasseAM,
  VV_SOMA_RECEITAOPERACIONAL.repasseNacioal,
  
  CASE
        WHEN CONS_TRIBUTOS.GC_ID = 4 THEN CONS_TRIBUTOS.VALOR_PAGO END as VALOR_NACIONAL ,
  CASE
        WHEN CONS_TRIBUTOS.GC_ID = 5 THEN CONS_TRIBUTOS.VALOR_PAGO END as VALOR_ESTADUAL,
  CASE 
  WHEN CONS_TRIBUTOS.GC_ID = 4 THEN (VV_SOMA_RECEITAOPERACIONAL.repasseNacioal - CONS_TRIBUTOS.VALOR_PAGO) END as VALOR_APAGAR_NACIONAL,

    CASE 
  WHEN CONS_TRIBUTOS.GC_ID = 5 THEN (VV_SOMA_RECEITAOPERACIONAL.repasseAM - CONS_TRIBUTOS.VALOR_PAGO) END as VALOR_APAGAR_ESTADUAL
  
From VV_SOMA_RECEITAOPERACIONAL
  Left Join CONS_TRIBUTOS On VV_SOMA_RECEITAOPERACIONAL.anoReferencia =
      CONS_TRIBUTOS.anoReferencia And VV_SOMA_RECEITAOPERACIONAL.mesReferencia =
      CONS_TRIBUTOS.mesReferencia
)
GO
